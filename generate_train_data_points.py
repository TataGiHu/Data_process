...
class DataGenerator():
  ...
  def get_closest_n_points(self, point, lane, num = 6):
    ...
    for i, reference_line_point in enumerate(reference_line_points):
          ...
          distance_of_this_point = abs(point[0] - car_point[0])
    ...
    for i in range(start_num, end_num + 1):
        enu_point = reference_line_points[i]
        enu_point = [enu_point["x"], enu_point["y"], enu_point["z"]]
        car_point = self.cc_.enu_to_car(enu_point)
  ...
  # def generate_gt_points(self, wm_lanes, dt):
  #   """[summary]
  ...
  #   """
  #   vision_perception_lanepoints = dt["lane_points"]
  #   for frame in vision_perception_lanepoints:
  #       for lane in frame:
    ...
    for j in range(i-self.n_frame+1, i+1): 
      ...
      for history_vision_lane in  history_vision_lanes:
        if is_ignore_vision_lane(history_vision_lane):
          continue
        current_lane_points = []
        for i in range(len(history_vision_lane["points_3d_x"])):
              current_lane_points.append([history_vision_lane["points_3d_x"][i], history_vision_lane["points_3d_y"][i], history_vision_lane["points_3d_z"][i]])
  ...
  for j, file_path in enumerate(file_list):
    ...
    for i, ori_data in enumerate(ori_datas):
      ...
      if i+1 >= n_frame: 
        ...
        gt = data_generator.generate_gt_points(wm_lanes, dt)
