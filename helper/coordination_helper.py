...
if sys.version_info[0] < 3:
  ...
  class CoordinateConverter: 
    ...
    def update(self, position_local, quaternion_local):
      car2enu_rot = R.from_quat([quaternion_local["x"], quaternion_local["y"], quaternion_local["z"], quaternion_local["w"]]).as_matrix()
      self.car2enu_transform = np.eye(4)
      self.car2enu_transform[:3,:3] = car2enu_rot
      self.car2enu_transform[:3,3] = np.array((position_local['x'], position_local['y'], position_local['z']))
      self.enu2car_transform = np.linalg.inv(self.car2enu_transform)


    def car_to_enu(self, position):
      return (self.car2enu_transform @ np.array(position))[:3]
    
    def enu_to_car(self, position):
      return (self.enu2car_transform @ np.array(position))[:3]
